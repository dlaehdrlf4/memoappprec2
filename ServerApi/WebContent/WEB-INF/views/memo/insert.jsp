...
<body>
	<form  enctype="multipart/form-data" method="post">
		...
		<table border="1" width="50%" height="80%" align='center'>
			...
			<tr>
				...
				<br /> <input type='file' id="image_path" name="image_path" /><br /></td>
			</tr>
			...
			<tr>
				...
				<td align="center" colspan="3">
					<p></p>
					<input type="button" value="메모저장" id="insertbtn"/>
	...
	<script>
		// 파일 선택 버튼에서 파일이 내용이 변경될 때 이미지 파일인 경우
		...
		document.getElementById("image_path").addEventListener("change",function(){
			...
			//이벤트 처리에서 this는 이벤트가 발생한 객체 document.getElementById("image_path") 이거다 this는
			if(this.files && this.files[0]){
				...
				var ext = filename.substr(idx+1, filename.length)
				*/
				//뒤에서 3글자 찾아오기
				var ext = filename.substr(filename.length-3, filename.length);
				//이미지 파일인지 여부를 저장할 변수
				var isCheck = false;
				if(ext.toLowerCase() == 'jpg' || ext.toLowerCase() == 'png' || ext.toLowerCase() == 'git'){
					isCheck = true;
				}else{
					alert("이미지 파일만 선택가능합니다.")
					
					//이 작업을 하지 않으면 선택한 파일의 경로가 남아있게 됩니다.
				...
				//파일의 내용을 읽기를 요청 - 비동기 요청: 항상 콜백을 생각해야 한다.
